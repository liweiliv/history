...
protected:
    ...
    POS_TYPE<GT_TYPE> *m_normals;
    g_color<GT_TYPE> * m_colors;
    GLubyte *m_indices;
...
public:
    g_sharp(uint32_t point_size,uint32_t index_size = 0,bool use_vbo = false)
    {
        ...
        if(index_size)
            m_mem_size=(sizeof(POS_TYPE<GT_TYPE>)+sizeof(POS_TYPE<GT_TYPE>))*m_point_size+(sizeof(g_color<GT_TYPE>)+sizeof(GLubyte))*index_size;
        else
            m_mem_size=(sizeof(POS_TYPE<GT_TYPE>)+sizeof(POS_TYPE<GT_TYPE>)+sizeof(g_color<GT_TYPE>))*m_point_size;
        m_mem = malloc(m_mem_size);
        m_vectors = (POS_TYPE<GT_TYPE> *)m_mem;
        m_normals = &m_vectors[m_point_size];
        m_colors = (g_color<GT_TYPE> *)&m_normals[m_point_size];
        if(index_size)
    ...
    g_sharp(const g_sharp<GT_TYPE,POS_TYPE> &s)
    {
        ...
        {
            assert(s.m_mem);
            assert(s.m_mem_size==(sizeof(POS_TYPE<GT_TYPE>)+sizeof(POS_TYPE<GT_TYPE>)+sizeof(g_color<GT_TYPE>))*s.m_point_size);
    ...
    void set_sharp(POS_TYPE<GT_TYPE> * vectors,POS_TYPE<GT_TYPE> *normals,g_color<GT_TYPE> colors,uint32_t point_size)
    {
        ...
        memcpy(m_normals,normals,sizeof(POS_TYPE<GT_TYPE>)*m_point_size);
        memcpy(m_colors,colors,sizeof(g_color<GT_TYPE>)*m_point_size);
    }
private:
    inline int draw_by_vbo(POS_TYPE<GT_TYPE> *pos,POS_TYPE<GT_TYPE> *dirct)
    {
        ...
        glNormalPointer(GT_TYPE, 0, (void*)(sizeof(POS_TYPE<GT_TYPE>)*m_point_size));
        glColorPointer(4, GT_TYPE, 0, (void*)((sizeof(POS_TYPE<GT_TYPE>+sizeof(POS_TYPE<GT_TYPE>))*m_point_size)));
