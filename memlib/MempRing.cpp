...
{
    c_init_chain(&p->head);
    if(NULL==(p->head_node=(memp_ring_node*)malloc(offsetof(memp_ring_node,buf)+MR_DEFAULT_BUF_SIZE)))
        return -1;
    c_insert_in_head(&p->head,&p->head_node->cn);
    p->head_node->buf_size=MR_DEFAULT_BUF_SIZE;
...
void * ring_alloc(memp_ring *p ,size_t size)
{
    ...
    else /*头部空间不足，尝试从下一个node取*/
    {
        ...
        if(n==p->end_node)
        {
            if(buf_size<MR_DEFAULT_BUF_SIZE)
                buf_size=MR_DEFAULT_BUF_SIZE;
        }
        else /*head之后的下一个node可以用*/
        {

            if(buf_size<MR_DEFAULT_BUF_SIZE&&n->buf_size>MR_DEFAULT_BUF_SIZE)/*如果空间太大，则收缩到默认值*/
                buf_size=MR_DEFAULT_BUF_SIZE;
