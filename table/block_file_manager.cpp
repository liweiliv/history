...
block_file_manager::iterator::iterator(iterator &itor)
{
    m_block=itor.m_block;
    if(m_block!=NULL)
        atomic_inc(&m_block->owners);
    m_manager=itor.m_manager;
}
block_file_manager::iterator::~iterator()
{
    if (m_block != NULL)
        atomic_sub(1, &m_block->owners);
}
...
{
	return atomic_read(&m_block->owners);
}
bool block_file_manager::iterator::next()
{
    ...
    if (&next->cn ==  &m_manager->m_head.head)
        return false;
    atomic_inc(&next->owners); //先增加next的引用计数,由于持有m_block的引用计数,删除只能从后往前，next是安全的
    atomic_dec(&m_block->owners); //释放掉当前的引用计数
    m_block = next;
    return true;
}
bool block_file_manager::iterator::prev()
{
    ...
    if (&prev->cn ==  &m_manager->m_head.head
            || __atomic_add_unless(&prev->owners, 1, -1) == -1)
        return false;
    atomic_dec(&m_block->owners);
    m_block = prev;
    return true;
}
...
block_file_manager::block_file_index_info * block_file_manager::_find(uint64_t id, int type)
{
    ...
    while (start <= end)
    {
        ...
        else
        {
            ...
            if(atomic_add_return(1,&m_index[idx]->owners)<=0)
            {
            	atomic_dec(&m_index[idx]->owners);
            	unlock();
            	return NULL;
            }
            unlock();
...
{
    if (atomic_read(&m_index_count) <= 1) //方便低锁设计，只剩下一个block时，不允许删除
        return 0;
    ...
    while (atomic_sub_return(1, &first->owners) != -1) //在确认first上没有使用者后，才可以开始删除
    {
        atomic_inc(&first->owners);
        usleep(1000);
    }
    //在有超过一个以上的block时，c_delete_node是安全的，但m_head的count将不准却，因为不会依赖count，所以无关紧要
    c_delete_node(&m_head, &first->cn);
    remove(first->file_name); //todo
    atomic_dec(&m_index_count);
    ...
    if (m_index_count.counter < m_index_volumn / 2 && m_index_count.counter > 4096)
    {
        block_file_index_info **tmp =
                (block_file_index_info**) malloc(sizeof(block_file_index_info*)
                        * (m_index_count.counter + m_index_count.counter / 2));
        m_index_volumn = m_index_count.counter + m_index_count.counter / 2;
        memcpy(tmp, &m_index[m_index_start_pos], sizeof(block_file_index_info*)
                * m_index_count.counter);
        free(m_index);
        m_index = tmp;
        m_index_start_pos = 0;
        m_index_end_pos = m_index_count.counter - 1;
    }
    else if (m_index_start_pos > m_index_volumn / 4 && m_index_start_pos > 256)
    {
        memcpy(m_index, &m_index[m_index_start_pos], m_index_count.counter
    ...
    {
    	m_clear_extern_info_func(first->extern_info);
...
int block_file_manager::load_index_file()
{
    ...
    for (;;)
    {
        ...
        while (remainder_size > 0)
        {
            ...
            atomic_inc(&m_index_count);
            remainder_size -= len + 1;
        }
    }
    return 0;
}
block_file_manager::block_file_manager(const char * index_filename,int (*clear_extern_info_func)(void*))
{
    c_init_chain(&m_head);
...
{
    while (atomic_read(&m_index_count) != 0)
    {
        rlock();
...
int block_file_manager::insert(const char *filename,void * extern_info)
{
    ...
    wlock();
    if (m_index_count.counter != 0 && id < m_index[m_index_end_pos]->file_id)
    {
        unlock();
        ring_free(&m_mp,b);
        return -1;
